...
function LoadAllFlowNode() {
    var NextNodesArray = curNextNodes.split(',');
    var FlowSetID = parseInt($("#hiddenFlowSetID").val());
    $.ajax({
        beforeSend: function () {
        },
        type: 'POST',
        ...
        success: function (data) {
            ...
            if (e.result == 'success') {
                ...
                for (var i = 0; i < e.ReturnObject.length; i++) {
                    if (e.ReturnObject[i].FlowSetID != FlowSetID) {
                        if (jQuery.inArray(e.ReturnObject[i].FlowSetID.toString(), NextNodesArray) == -1) {
                            $("#selEnableNodes").append('<option value="' + e.ReturnObject[i].FlowSetID + '">' + e.ReturnObject[i].NodeName + '</option>');
                        }
                        else {
                            $("#selNextNodes").append('<option value="' + e.ReturnObject[i].FlowSetID + '">' + e.ReturnObject[i].NodeName + '</option>');
                        }
                    }
                }
            }
...
function Save(flownodegrid, dialog) {
    var FlowSetID = $("#hiddenFlowSetID").val();
    ...
    var FlowSetObj = { "FlowSetID": FlowSetID, "NodeName": NodeName, "DisplayName": DisplayName, "SortID": SortID, "GroupID": GroupID, "NextNodes": ArrayNextNodes.join() };
    ...
    $.ajax({
        ...
        type: 'POST',
        async: false, 
        url: RootPath + '/FlowSet/EditFlowSetAjax/?rnd=' + Math.random(),
        dataType: "json",
        contentType: 'application/json; charset=utf-8',
        data: $.toJSON({ FlowNode: FlowSetObj,FlowConfig:FlowConfigObj }),
        success: function (e) {
            if (e.result == 'success') {
                msgdialog = $.ligerDialog.waitting('编辑审稿流程步骤成功!');
                setTimeout(function () {
                    msgdialog.close();
                }, 2000);
