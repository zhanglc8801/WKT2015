...
namespace WKT.Service.Wrapper
{
    ...
    public class ServiceContainer
    {
        ...
        {
            configFile = WKT.Config.BaseConfig.ServiceUnityConfigPath;
            if (string.IsNullOrEmpty(configFile))
            {
                throw new Exception("请检查配置文件目录(config)中是否正确配置unity.service.config文件");
            }
            else
            {
