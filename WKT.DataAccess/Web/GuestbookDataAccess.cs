...
namespace WKT.DataAccess
{
    ...
    public partial class GuestbookDataAccess
    {
        ...
        public GuestbookEntity GetGuestbook(Int64 messageID)
        {
            ...
            sqlCommandText.Append("SELECT TOP 1  MessageID,JournalID,PMessageID,UserName,Email,Tel,Title,MessageContent,ReplyContent,IP,AddDate FROM dbo.Guestbook WITH(NOLOCK)");
        ...
        public List<GuestbookEntity> GetGuestbookList()
        {
            ...
            StringBuilder sqlCommandText = new StringBuilder();
            sqlCommandText.Append("SELECT  MessageID,JournalID,PMessageID,UserName,Email,Tel,Title,MessageContent,ReplyContent,IP,AddDate FROM dbo.Guestbook WITH(NOLOCK)");
        ...
        {
            int recordCount = 0;
            DataSet ds = db.GetPagingData("Guestbook","MessageID,JournalID,PMessageID,UserName,Email,Tel,Title,MessageContent,ReplyContent,IP,AddDate",query.Order,query.Where,query.CurrentPage,query.PageSize,out recordCount);
        ...
        {
            int recordCount=0;
            DataSet ds = db.GetPagingData("Guestbook","MessageID,JournalID,PMessageID,UserName,Email,Tel,Title,MessageContent,ReplyContent,IP,AddDate"," MessageID DESC","",query.CurrentPage,query.PageSize,out recordCount);
            Pager<GuestbookEntity>  pager=new Pager<GuestbookEntity>();
            if(ds!=null && ds.Tables.Count>0)
            {
                 pager.ItemList= MakeGuestbookList(ds.Tables[0]);
        ...
        public Pager<GuestbookEntity> GetGuestbookPageList(GuestbookQuery query)
        {
            ...
            string whereSQL=GuestbookQueryToSQLWhere(query);
            string orderBy=GuestbookQueryToSQLOrder(query);
            DataSet ds=db.GetPagingData("Guestbook","MessageID,JournalID,PMessageID,UserName,Email,Tel,Title,MessageContent,ReplyContent,IP,AddDate",orderBy,whereSQL,query.CurrentPage,query.PageSize,out recordCount);
            Pager<GuestbookEntity>  pager=new Pager<GuestbookEntity>();
            if(ds!=null && ds.Tables.Count>0)
            {
                 pager.ItemList= MakeGuestbookList(ds.Tables[0]);
